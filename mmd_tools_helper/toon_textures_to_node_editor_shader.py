    ...
    if o.type == "MESH":
        ...
        if o.data.materials is not None:
            for m in o.data.materials:
                ...
                mix_rgb_node_ramp_overlay.inputs["Color2"].default_value = (
                    1.0,
                    1.0,
                    1.0,
                    1.0,
                ...
                mix_rgb_node.inputs["Color2"].default_value = (
                    m.diffuse_color[0],
                    m.diffuse_color[1],
                    m.diffuse_color[2],
                    1.0,
                ...
                m.node_tree.links.new(
                    ...
                    toon_texture_color_ramp.outputs["Color"],
                )
                print(len(m.node_tree.links))
                m.node_tree.links.new(
                    mix_rgb_node_ramp_overlay.inputs["Fac"],
                    toon_texture_color_ramp.outputs["Alpha"],
                )
                print(len(m.node_tree.links))
                m.node_tree.links.new(
                    mix_rgb_node.inputs["Color1"],
                    mix_rgb_node_ramp_overlay.outputs["Color"],
                )

                print(len(m.node_tree.links))
                m.node_tree.links.new(
                    mix_rgb_node_add_sphere.inputs["Color1"],
                    mix_rgb_node.outputs["Color"],
                )
                print(len(m.node_tree.links))
                m.node_tree.links.new(
                    output_node.inputs["Color"],
                    mix_rgb_node_add_sphere.outputs["Color"],
                )
                print(len(m.node_tree.links))
                m.node_tree.links.new(
                    diffuse_texture_node.inputs["Vector"],
                ...
                m.node_tree.links.new(
                    ...
                    sphere_texture_geometry_normal_node.outputs["Normal"],
                )
                print(len(m.node_tree.links))
                m.node_tree.links.new(
                    mix_rgb_node_add_sphere.inputs["Color2"],
                    sphere_texture_node.outputs["Color"],
                )
                            ...
                            if t == 1:
                                if m.texture_slots[t].texture.type == "IMAGE":
                                    if m.texture_slots[t].texture.image is not None:
                                        ...
                                        toon_image_to_color_ramp(
                                            toon_texture_color_ramp,
                                            m.texture_slots[t].texture.image,
                                        )
